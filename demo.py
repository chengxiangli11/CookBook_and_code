...
# function
def spider(type, id):
    url = 'https://www.kuaidi100.com/query?type={0}&postid={1}'.format (type, id)
    ...
    if json['status'] == "200":
        ...
        print("//////////////快递详细信息//////////////")
        for x in data_json:
            print("{0} : {1}" .format(x['time'], x['context']))
    else:
        print("错误的快递单号!")
...
def express_type_get():
    express_type = (
    'shunfeng', 'yunda', 'shentong', 'yuantong', 'zhongtong', 'ems', 'tiantian', 'huitongkuaidi', 'quanfengkuaidi',
    'youzhengguonei')
    print(
